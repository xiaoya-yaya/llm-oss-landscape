...
TEMP_NEW_PROJECTS = os.path.join(BASE, "data", "new_projects_pending.json")
...
"""
    ...
    for i, p in enumerate(new_projects, 1):
        desc = (p.get("description", "") or "")[:50] + ("..." if len(p.get("description", "") or "") > 50 else "")
        topics = p.get("topics", "")[:40] + ("..." if len(p.get("topics", "") or "") > 40 else "")
        stars = p.get("stars", 0)
...
"""
            for p, score, reason in recommendations[:5]:  # Top 5
                ...
                report += f"| {p['repo_name']} | {rating} | {reason} |\n"
...
def generate_reader_report(new_projects, trend_months, recommendations=None):
    ...
    report += "|------|------|-------|------|\n"
    for p in new_projects:
        desc = (p.get("description", "") or "")[:60] + ("..." if len(p.get("description", "") or "") > 60 else "")
        stars = p.get("stars", 0)
        stars_str = f"{stars/1000:.1f}k" if stars >= 1000 else str(stars)
        lang = p.get("language", "-") or "-"
...
def publish_to_yuque(reader_report):
    """Save reader-facing report for Yuque publishing. Returns None (publishing done by Claude session via MCP)."""
    date_str = datetime.now().strftime('%Y-%m-%d')
    ...
    print(f"{CYAN}Publish to Yuque via: skylark_doc_create(book_id={YUQUE_BOOK_ID}, title='{title}'){RESET}")
    return None
...
def create_pr(projects):
    ...
    pending_file = os.path.join(BASE, "data", "weekly_pending.json")
    ...
    try:
        ...
        subprocess.run(["git", "add", REPORT_FILE, pending_file], check=True, capture_output=True)
        ...
        for p in projects:
            ...
            stars_str = f"{stars/1000:.1f}k" if stars >= 1000 else str(stars)
            lang = p.get("language", "-") or "-"
            body_lines.append(f"- [ ] **{p['repo_name']}** — {desc} ⭐{stars_str} ({lang})")

        body = "\n".join(body_lines)

        result = subprocess.run(
            ["gh", "pr", "create", "--title", f"Weekly update {date_str}: {len(projects)} new projects",
             "--body", body],
...
def parse_pr_checklist(pr_body):
    """Parse checked/unchecked items from PR body. Returns list of (repo_name, checked)."""
    items = []
...
def run_check(full_mode=False):
    ...
    yuque_url = publish_to_yuque(reader_report)

    # Send DingTalk with Yuque link
    send_dingtalk(reader_report, new_agentic_projects, recommendations, yuque_url)
...
def run_post_merge():
    ...
    # Load pending data from merged branch via git
    ...
    if os.path.exists(pending_file):
        ...
        print(f"Fetching pending data from merged branch {branch_name}...")
        try:
            result = subprocess.run(
                ["git", "show", f"{branch_name}:data/weekly_pending.json"],
                capture_output=True, text=True, check=True
            )
            all_pending = json.loads(result.stdout)
        except (subprocess.CalledProcessError, json.JSONDecodeError) as e:
            print(f"{RED}Could not fetch pending data from branch: {e}{RESET}")
            return
    else:
        print(f"{RED}No pending data available{RESET}")
        return
